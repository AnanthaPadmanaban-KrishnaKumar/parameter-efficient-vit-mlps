...
from parameter_reduction import ThinMLPStrategy, ShallowMLPStrategy, GroupedMLPStrategy
from parameter_reduction.strategies import dg_vit
...
# Parameter Reduction parameters
group = parser.add_argument_group('Parameter reduction')
group.add_argument('--reduction-strategy', type=str, default=None,
                   choices=['thin', 'shallow', 'grouped'],
                   help='Parameter reduction strategy to apply')

group.add_argument('--reduction-mlp-ratio', type=float, default=2.0,
                   help='MLP ratio for thin (default: 2.0), pruning ratio for shallow (default: 0.5), or scale factor for grouped (0.707 or 1.0)')
...
def main():
    ...
    # BEFORE model creation, handle ThinMLP
    pre_creation_strategy = None
    if args.reduction_strategy == 'thin':
        pre_creation_strategy = ThinMLPStrategy(target_mlp_ratio=args.reduction_mlp_ratio)
        # Merge reduction kwargs with existing model kwargs
        reduction_kwargs = pre_creation_strategy.get_model_kwargs_modifier()
        args.model_kwargs = {**args.model_kwargs, **reduction_kwargs}
        
        if utils.is_primary(args):
            _logger.info(f'Will apply ThinMLP reduction with mlp_ratio={args.reduction_mlp_ratio}')
    ...
    # AFTER model creation, handle ShallowMLP and GroupedMLP
    post_creation_strategy = None
    if args.reduction_strategy == 'shallow':
        post_creation_strategy = ShallowMLPStrategy(target_ratio=args.reduction_mlp_ratio)
        if utils.is_primary(args):
            _logger.info(f'Applying ShallowMLP reduction with ratio={args.reduction_mlp_ratio}')
        model = post_creation_strategy.apply(model)
    elif args.reduction_strategy == 'grouped':
        post_creation_strategy = GroupedMLPStrategy(scale_factor=args.reduction_mlp_ratio)
        if utils.is_primary(args):
            scale_desc = "1/√2" if abs(args.reduction_mlp_ratio - 0.707) < 0.01 else str(args.reduction_mlp_ratio)
            _logger.info(f'Applying GroupedMLP reduction with scale={scale_desc}')
        model = post_creation_strategy.apply(model)

    # Log stats for whichever strategy was applied
    if pre_creation_strategy and utils.is_primary(args):
        pre_creation_strategy.log_stats(model, _logger)
    elif post_creation_strategy and utils.is_primary(args):
        post_creation_strategy.log_stats(model, _logger)
